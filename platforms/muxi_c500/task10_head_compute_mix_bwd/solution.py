...
def _head_compute_mix_bwd_kernel(
    ...
    tl.store(grad_scale_ptr, grad_scale)
    tl.store(grad_base_ptr, grad_base0)
    tl.store(grad_base_ptr + 1, grad_base1)
    tl.store(grad_base_ptr + 2, grad_base2)
    tl.store(grad_base_ptr + 3, grad_base3)
...
class ModelNew(nn.Module):
    ...
    def forward(
        ...
        _head_compute_mix_bwd_kernel[(1,)](
            input_mix,
            mhc_scale,
            mhc_base,
            grad_out,
