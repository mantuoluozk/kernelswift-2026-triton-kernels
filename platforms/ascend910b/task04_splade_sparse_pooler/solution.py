...
class ModelNew(nn.Module):
    def __init__(self, hidden_size: int = 768, vocab_size: int = 30522, pooling: str = "max"):
        ...
        self.pool_block_n = 2048
        self.pool_num_warps = 4
        # The fused 32x64 decoder/pool kernel exceeds the Ascend AICore
        # watchdog for this 30k-vocabulary workload. Keep the two lightweight
        # Triton fusions and let torch_npu execute the decoder GEMM.
    ...
    def forward(self, hidden_states: torch.Tensor, seq_lens: torch.Tensor) -> list:
        ...
        normalized = torch.empty_like(dense_output)
        _gelu_layer_norm_kernel[(hidden_states.shape[0],)](
            dense_output,
        ...
        if self.direct_decoder_pool:
            ...
            _decoder_splade_max_kernel[(4, triton.cdiv(30522, self.direct_block_n))](
                normalized,
                self.decoder.weight,
                self.decoder.bias,
                output,
