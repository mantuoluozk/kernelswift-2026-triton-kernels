...
- 每个 program 处理一个 batch/head 和 32-query tile。
...
| --- | ---: | ---: | ---: |
| 通过 | 0.349675 ms | 0.300920 ms | 1.162x |

关键配置：`BLOCK_M=32`、`BLOCK_N=32`、`num_warps=1`。

## 复现

```bash
cd /data/kernelswift-2026-triton-kernels/platforms/ascend910b/task06_mm_encoder_attention
source ../setup_env.sh
