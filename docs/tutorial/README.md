# 从零学习 Triton 算子开发与优化：BW1000 十题实战
...
教程中的代码、形状和性能数据均来自本仓库在海光 BW1000 上完成的 10 个任务。建议一边阅读，一边打开对应的 `reference.py`、`solution.py` 和 `benchmark.py` 对照。
...
5. 按下面的任务顺序学习。前几题偏融合与归约，中间进入 GEMM 和 Attention，后几题集中训练小矩阵、随机数和反向归约。
